...
public class MainActivity extends AppCompatActivity {
    ...
    private EditText passportNumberView;//document no
    private EditText expirationDateView;//expire date
    private EditText birthDateView;//birth date
    ...
    private class ReadTask extends AsyncTask<Void, Void, Exception> {
        ...
        protected void onPostExecute(Exception result) {
            ...
            if (result == null) {
                ...
                List<String> addres = dg11File.getPermanentAddress();
                String newAddress = "";
                if(addres!=null)
                {
                    for (int i = 0; i < addres.size(); i++)
                    {
                        newAddress = newAddress + addres.get(i);
                    }
                }

                intent.putExtra(ResultActivity.KEY_ADRESS,newAddress);
                intent.putExtra(ResultActivity.KEY_PHONE, dg11File.getTelephone());

                List<String> Bplaces = dg11File.getPlaceOfBirth();
                String Bplace = "";
                if(Bplaces!=null)
                {
                    for (int i = 0; i < Bplaces.size(); i++)
                    {
                        Bplace = Bplace + Bplaces.get(i);
                    }
                }
                intent.putExtra(ResultActivity.KEY_BIRTH_PLACE, Bplace);
